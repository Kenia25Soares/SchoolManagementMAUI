...
namespace SchoolManagementMAUI.Services
{
    public class ApiProfileService : IProfileService
    {
        ...
        public async Task<UpdateFullProfileResult> UpdateFullProfileAsync(ProfileUpdateData updateData, string token)
        {
            ...
            if (!string.IsNullOrWhiteSpace(updateData.PhoneNumber)) form.Add(new StringContent(updateData.PhoneNumber), "phoneNumber");
            if (!string.IsNullOrWhiteSpace(updateData.Email)) form.Add(new StringContent(updateData.Email), "email");
            if (updateData.DateOfBirth.HasValue) form.Add(new StringContent(updateData.DateOfBirth.Value.ToString("yyyy-MM-dd")), "dateOfBirth");
            if (!string.IsNullOrWhiteSpace(updateData.Address)) form.Add(new StringContent(updateData.Address), "address");
